...
use Nacosvel\Mitt\Emitter;
use Nacosvel\Mitt\Facades;
...
class EmitterTest extends TestCase
{
    ...
    {
        $eventKey1 = Facades\Emitter::getEventKey('on');
        $eventKey2 = $this->emitter->getEventKey('on');
        $eventKey3 = $this->emit->getEventKey('on');

        Facades\Emitter::on('on', function () {
            return 'on_static';
        });
        ...
        $this->assertTrue(Facades\Emitter::emit('on,') == ['on_static']);
        $this->assertTrue($this->emitter->emit(',on') == ['on_emitter', 'on_emitter']);
        $this->assertTrue($this->emit->emit(['on']) == ['on_emit']);

        $this->assertTrue(Facades\Emitter::getEventsKeys($eventKey1) == [$eventKey1]);
        ...
        $this->assertTrue(Facades\Emitter::getEventsKeys() == [$eventKey1]);
        $this->assertTrue($this->emitter->getEventsKeys() == [$eventKey2]);
